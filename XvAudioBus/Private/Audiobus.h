...
#define ABSDKVersionString @"2.3.2"
...
@page Be-ready-for-audiobus-3 Be ready for Audiobus 3
 
 You know all of these little hiccups occurring with Inter-App Audio and Core MIDI. Apps remaining as zombie processes in the background, Inter-App audio units returning errors and needing to be (re-)launched manually, synth apps receiving MIDI events twice because they connect to a MIDI controller app and the MIDI controller app connecting to them as well. We could go on... Experienced users will find ways to work around of these things. But there is a large group of people are startled by them, lose confidence in iOS apps and therefore stops losing interest in music apps altogether.
 
 
 Let’s fix this, together. Let’s make sure that iOS music making platform becomes rock solid. We think users will make more music and enjoy the process more. It is going to be beneficial to all of us. Users and developers.
 
 
 With Audiobus 3 we are aiming to solve the issues mentioned above. As you might have expected, we need your help. As usual, we’ve gone to great lengths to make sure we’re not wasting your time.
 
 
 You need to update your App to the Audiobus SDK to the new version 2.4. The SDK contains workarounds for the following problems:
 
 <ul>
 <li> <b>Zombie processes:</b> If your app has been launched in the background via Inter-App audio and has never been in the foreground, it will be terminated by Audiobus 3. This saves battery life and removes errors occurring when you try to load your app in a host the next time.</li>
 <li><b>Inter-App audio errors:</b> The most frequently reason for Inter-App audio errors are not safely terminated node apps. With the possibility to terminate disconnected apps we can remove a significant amount of occurrences.
 <li><b>Open App Alert:</b> When you try to open an app via Audiobus connection panel the first time, an ugly alert appears, asking you for permission. When connected to Audiobus 3 your app will use the Inter-App Audio launch URL which removes the mentioned alert.</li>
 <li><b>Double MIDI Routings:</b> In iOS it can easily happen that an app receives MIDI events twice. To prevent this we have added a flag to the SDK telling your app to enable or disable sending or receiving MIDI via Core MIDI. Audiobus 3 will switch on or off this flag, depending on the MIDI routing done.</li>
 <li><b>Inter-App Audio Transport Panel:</b> Audiobus 3 will launch your app in the background. This means at the first place Inter-App audio will be connected. After that Audiobus 3 will connect. In many cases this will make your app show both at the same time: The inter-app audio transport panel as well the Audiobus connection panel. To prevent this error we have added a property to the SDK, telling you exactly when to hide the Inter-App Audio Transport Panel.</li>
 </ul>

 
@section audiobus3-five-simple-steps Five simple steps you need to do
 <ol>
 <li>Update your App to Audiobus SDK 2.4.</li>
 <li>Listen to ABApplicationWillTerminateNotification.</li>
 <li>Observe and react to shouldHideInterAppAudioTransportPanel.</li>
 <li>Observe and react to shouldDisableCoreMIDIReceive.</li>
 <li>Test and upload a new version of your app to the app store</li>
 </ol>
 
 @subsection audiobus3-five-simple-steps-step-1 Update your App to Audiobus SDK 2.4
 The Audiobus SDK 2.4 <a href="http://developer.audiob.us/download/Audiobus-SDK-2.4b1.zip">can be downloaded here</a>. Simply replace the existing files by the new ones.
 
 
 @subsection audiobus3-five-simple-steps-step-2 Observe ABApplicationWillTerminateNotification
 
 If your app needs to do something before Audiobus 3 terminates the zombie process of your app, listen to ABApplicationWillTerminateNotification:
 
 @code
    [[NSNotificationCenter defaultCenter]
     addObserverForName:ABApplicationWillTerminateNotification
     object:0
     queue:0
     usingBlock:^(NSNotification * _Nonnull note) {
         //Do things to be done before termination
     }];
 @endcode
 
 @subsection audiobus3-five-simple-steps-step-3 Observe shouldHideInterAppAudioTransportPanel:
 
 If your app shows an Inter-App Audio transport panel, observe the “shouldHideInterAppAudioTransportPanel” property of the AudiobusController. Audiobus will set this flag if it shows the connection panel. In that case the inter-app audio transport panel needs to be hidden.
 
 First you need to define a context object:
 
 @code
 static void *kIAATransPortPanelStateChangedContext = &kIAATransPortPanelStateChangedContext;
 @endcode
 
 Then observe the property, e.g. in your application's launch function:
 
 @code
 - (BOOL)application:(UIApplication *)application didFinishLaunchingWithOptions:(NSDictionary *)launchOptions {
    //...

    [_audioEngine.audiobusController addObserver:self
                                      forKeyPath:@"shouldHideInterAppAudioTransportPanel"
                                         options:0
                                         context:kIAATransPortPanelStateChangedContext];
    //...
 }
 @endcode
 
 And finally implement / extend the observeValueForKeyPath method:
 ...
 - (void)observeValueForKeyPath:(NSString *)keyPath
    ...
    else if(context == kIAATransPortPanelStateChangedContext){
        BOOL hideConnectionPanel = _audioEngine.audiobusController.shouldHideInterAppAudioTransportPanel;
        NSString *showHide = hideConnectionPanel ? @"hidden" : @"shown";
        NSLog(@"Todo: Transport panel should be %@ now.", showHide);
    }
    else if(context == ...){
        //...
    }
...
@subsection audiobus3-five-simple-steps-step-4 Observe and react to shouldDisableCoreMIDIReceive:
 
 The following figure shows, why receiving Core MIDI should be disabled sometimes: A MIDI source, e.g. MIDI keyboard is connected to your iOS device. Audiobus 3 will receive the MIDI events, route it through MIDI effects and then forward it to your app, e.g. a synth app. At the same time your app is listening via Core MIDI to the connected keyboard too. The MIDI event would be received twice by your app: A first time from Audiobus and a second time directly via Core MIDI. These double routings are very obscure and hardly to detect by inexperienced users.
 
  <img src="disableReceivingCoreMIDI.png" width="570" title="Core MIDI double routings" />
 
 To prevent such double routings we have added a new property to the Audiobus Controller, called shouldDisableCoreMIDIReceive. Please observe this property and disable receiving Core MIDI when it becomes true.

 Again you need to define an context first:
 
 @code
 static void *kShouldReceiveCoremIDIContext = &kShouldReceiveCoremIDIContext;
 @endcode

 Then you need to observe the property:
 ...
 @code
  - (BOOL)application:(UIApplication *)application didFinishLaunchingWithOptions:(NSDictionary *)launchOptions {
      ...
      [_audioEngine.audiobusController addObserver:self
                                       forKeyPath:@"shouldDisableCoreMIDIReceive"
                                          options:0
                                          context:kShouldReceiveCoremIDIContext];
      //...
  }
 @endcode
 
 And finally you need to extend observeValueForKeyPath and react to the changes:
 ...
 @code
  - (void)observeValueForKeyPath:(NSString *)keyPath
    ...
    else if(context == kShouldReceiveCoremIDIContext){
        BOOL disableCoreMIDI = _audioEngine.audiobusController.shouldDisableCoreMIDIReceive;
        NSString *disableEnable = disableCoreMIDI ? @"hidden" : @"shown";
        NSLog(@"Todo: Receiving Core MIDI should be %@ now.", disableEnable);
    }
    else if(context == ...){
        //...
    }
 ...
 @subsection audiobus3-five-simple-steps-step-5 Test and upload a new version to the AppStore
 
 To test if your app is working correctly with Audiobus 3, please contact 
 <a href="mailto:sebastian@audiob.us">sebastian@audiob.us</a> .
 Sebastian will sent you a TestFlight invitation to Audiobus 3 where you can check, 
 if the following things work properly:
 
 <ul>
   <li> The Inter-App audio transport panel is hidden once Audiobus is connected.</li>
   <li>Audiobus 3 does launch your app in the background correctly.</li>
   <li>When connected to Audiobus your app receives no MIDI events via Core MIDI. 
   Your app is only receiving MIDI events using Core MIDI.</li>
 </ul>
...
@page Migration-Guide 1.x-2.x Migration Guide
 ...
 Note that for technical reasons the 2.x SDK is supported on iOS 8.0 and up only.
